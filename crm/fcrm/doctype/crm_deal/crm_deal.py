...
class CRMDeal(Document):
	...
	def update_exchange_rate(self):
		if self.has_value_changed("currency") or not self.exchange_rate:
			...
			# If no currency set or same as system currency → 1.0
			if not self.currency or self.currency == system_currency:
				self.db_set("exchange_rate", 1.0)
				return

			exchange_rate = 1
			try:
				exchange_rate = get_exchange_rate(self.currency, system_currency, frappe.utils.nowdate()) or 1.0
			except Exception:
				# ultra-safe fallback; don't block save
				exchange_rate = 1.0
