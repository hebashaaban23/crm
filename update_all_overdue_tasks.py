"""
سكريبت لتحويل جميع المهام المتأخرة إلى Backlog
للتشغيل: bench --site your-site console
ثم: exec(open('apps/crm/update_all_overdue_tasks.py').read())
"""
...
if len(tasks) == 0:
    ...
    for task in tasks:
        try:
            ...
            updated += 1
            print(f"✅ {task.name}: {task.title[:50]}... ({task.status} → Backlog)")
