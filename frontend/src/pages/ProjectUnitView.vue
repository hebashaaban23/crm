<template>
  <LayoutHeader>
    <!-- LEFT HEADER: Inventory / Project Name / Unit Name -->
    <template #left-header>
      <div class="text-base sm:text-lg font-semibold flex items-center gap-2">
        ...
        <template v-if="unit?.project">
          <RouterLink
            :to="{ name: 'ProjectView', params: { project: unit.project } }"
  ...
  <!-- HERO (matches ProjectView; half-height spacer) -->
  ...
  <div v-if="activeTab==='details'" class="p-4 space-y-6">
    ...
    <Card>
      ...
      <template #content>
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          <div>
            <div class="text-xs text-gray-500 mb-0.5">{{ __('Project') }}</div>
            <div class="rounded-lg border px-3 py-2 bg-white/50 dark:bg-gray-900/50 min-h-[40px]">
              {{ projectTitle || '—' }}
            </div>
          </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <Card v-if="unit?.floor_plan">
        <template #header>
          <div class="flex items-center gap-2">
            <FeatherIcon name="map" class="h-4" />
            <span class="font-semibold">{{ __('Floor Plan') }}</span>
          </div>
        </template>
        <template #content>
          <a :href="unit.floor_plan" target="_blank" rel="noopener" class="inline-flex items-center gap-2 text-primary-600 hover:underline">
            <FeatherIcon name="download" class="h-4" /> {{ __('Open / Download') }}
          </a>
        </template>
      </Card>

      <Card v-if="unit?.brochure">
        <template #header>
          <div class="flex items-center gap-2">
        ...
        <template #content>
          <a :href="unit.brochure" target="_blank" rel="noopener" class="inline-flex items-center gap-2 text-primary-600 hover:underline">
            <FeatherIcon name="download" class="h-4" /> {{ __('Open / Download') }}
          </a>
        </template>
      </Card>
      ...
      <Card v-if="videoUrl">
        <template #header>
          <div class="flex items-center gap-2">
            <FeatherIcon name="play-circle" class="h-4" />
            <span class="font-semibold">{{ __('Video') }}</span>
          </div>
        </template>
        <template #content>
          <a :href="videoUrl" target="_blank" rel="noopener" class="inline-flex items-center gap-2 text-primary-600 hover:underline">
            <FeatherIcon name="external-link" class="h-4" /> {{ videoUrl }}
          </a>
        </template>
      </Card>
    </div>
...
const projectTitle = computed(() => projectDoc.value?.project_name || projectDoc.value?.name || unit.value?.project || '—')
...
async function fetchUnit() {
  ...
  try {
    const doc = await call('frappe.client.get', { doctype: 'Project Unit', name: unitNameParam })
    unit.value = doc || null
    gallery.value = Array.isArray(doc?.gallery) ? mapGallery(doc.gallery) : []

    if (doc?.project) {
      try {
        const proj = await call('frappe.client.get', { doctype: 'Real Estate Project', name: doc.project })
        projectDoc.value = proj || null
      } catch {
        projectDoc.value = null
      }
    }
