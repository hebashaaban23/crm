<template>
  <LayoutHeader v-if="deal.data">
    ...
    <template #right-header>
      ...
      <AssignTo v-model="assignees.data" :data="document.doc" doctype="CRM Deal" />
      <Dropdown
        v-if="document.doc"
        :options="statusOptionsSimple(CUSTOM_DEAL_STATUSES, triggerStatusChange)"
      >
        <template #default="{ open }">
          <Button :label="document.doc.status">
            <template #prefix>
              <IndicatorIcon :class="getCustomDealStatus(document.doc.status).color" />
            </template>
            <template #suffix>
  ...
  <div v-if="deal.data" class="flex h-full overflow-hidden">
    ...
    <Resizer side="right" class="flex flex-col justify-between border-l">
      ...
      <!-- Organization block removed; show title only -->
  ...
  <!-- DeleteLinkedDocModal removed -->
  <!-- We now use a simple confirm in confirmAndDeleteDeal() -->
  <LostReasonModal
...
function getCustomDealStatus(status) {
  if (status === 'Made Reservation') return { color: 'orange' }
  if (status === 'Deal Done') return { color: 'green' }
  return getDealStatus(status) || { color: '' }
}
function statusOptionsSimple(list, onChange) {
  return list.map((s) => ({
    label: s,
    prefix: () => h(IndicatorIcon, { class: getCustomDealStatus(s).color }),
    onClick: () => onChange(s),
  }))
}
...
function confirmAndDeleteDeal() {
  // Simple, dependency-free confirm. Replace with a fancy dialog if you prefer.
  if (window.confirm(__('Delete this deal permanently?'))) {
...
function setLostReason() {
  // No Lost workflow needed for your two statuses; just save
