<template>
  ...
  <DealModal v-if="showDealModal" v-model="showDealModal" :defaults="defaults" />
...
import { Tooltip, Avatar, Dropdown, Badge, Button } from 'frappe-ui'
import { useRoute } from 'vue-router'
import { ref, reactive, computed, h } from 'vue'

const { getFormattedPercent, getFormattedFloat, getFormattedCurrency } = getMeta('CRM Deal')
...
function getGroupedByRows(listRows, groupByField, columns) {
  ...
  groupByField.options?.forEach((option) => {
    ...
    if (groupByField.fieldname == 'status') {
      groupDetail.icon = () =>
        h(IndicatorIcon, {
          class: getCustomDealStatus(option)?.color,
        })
    }
  ...
  return rows.map((deal) => {
    ...
    deals.value.data.rows.forEach((row) => {
      ...
      if (row === 'website') {
        ...
        const s = deal.status
        const color = getCustomDealStatus(s).color
        _rows[row] = { label: s, color }
        ...
        // Hide organization: blank label so it won't render
        _rows[row] = { label: '' }
      }
    })
...
function showNote(name) { docname.value = name; showNoteModal.value = true }
...
function showTask(name) { docname.value = name; showTaskModal.value = true }
</script>
