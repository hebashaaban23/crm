...
const fields = computed(() => {
  ...
  if (gridViewSettings.length) {
    let d = gridViewSettings
      .map((gs) => getFieldObj(gridFields.find((f) => f.fieldname === gs.fieldname)))
      .filter(Boolean) // drop any nulls just in case
    return d
  }
  return (
    gridFields
      ?.filter((f) => f && f.in_list_view)
      .map((f) => getFieldObj(f))
      .filter(Boolean) || []
  )
})
...
function getFieldObj(field) {
  if (!field) {
    // Return a harmless placeholder to avoid crashes
   return { fieldname: '__unknown__', fieldtype: 'Data', label: '', in_list_view: false }
  }
